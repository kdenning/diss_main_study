...
# The person average seems pretty variable, I'm not sure what is going on; but maybe it is too even?
clean_data %>% 
  group_by(sub_id, bfi_self) %>% 
  mutate(mean = mean(bfi_self)) %>%
  ungroup() %>% 
  select(sub_id, mean) %>% 
  unique() %>% 
  group_by(mean) %>% 
...
```{r}
get_outlier_number(clean_data$bfi_self)
```

#### Stereo

```{r}
get_outlier_number(clean_data$bfi_stereo)
```

#### Target

```{r}
get_outlier_number(clean_data$bfi_targ)
```
...
```{r}
get_outlier_number(clean_data$eli_stereo)
```

#### Target

```{r}
get_outlier_number(clean_data$eli_targ)
```
...
```{r}
get_outlier_number(clean_data$realistic_q)
```

#### Symbolic

```{r}
get_outlier_number(clean_data$symbolic_q)
```

#### Distance - coffee

```{r}
get_outlier_number(clean_data$distance_coffee)
```

#### Distance - town

```{r}
get_outlier_number(clean_data$distance_town)
```

#### Explicit - target

```{r}
get_outlier_number(clean_data$explicit_targ)
```

#### Explicit - group

```{r}
get_outlier_number(clean_data$explicit_group)
```
...
```{r}
get_outlier_number(clean_data$pol_orient_1)
```

### Social

```{r}
outlier_ind1 <- get_outlier_number(clean_data$pol_orient_2)

orient_2_outliers <- clean_data[outlier_ind1,]
orient_2_outliers %>% 
...
```{r}
outlier_ind2 <- get_outlier_number(clean_data$pol_orient_3)

orient_3_outliers <- clean_data[outlier_ind2,]
orient_3_outliers %>% 
  select(sub_id, pol_orient_2) %>% 
  unique()
```
...
clean_data %>% 
  select(sub_id, analog_condition, target_condition, distance_coffee,
         ...
         bfi_stereo, bfi_targ, eli_self, eli_stereo, eli_targ) %>% 
  na.omit() %>% 
  select(sub_id) %>% 
  unique() %>% 
  nrow()
