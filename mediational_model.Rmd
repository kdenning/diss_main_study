...
data_4_med_bfi <- clean_data_bfi %>% 
  ...
  df_coeffs <- data.frame(matrix(vector(mode = 'numeric',length = 1), nrow = 1, ncol = 16))
...
mclapply(1:5, boot_funct)  

set.seed(123)
trials <- seq(1, 5)
system.time({
  list_results <- lapply(trials, boot_funct)
}) 

med_results <- bind_rows(list_results, .id = "model")
