...
No predictors included; just projection
...
l1_resid_mod <- lmer(bfi_targ_pmc ~ bfi_self_pmc+ 
                     (bfi_self_pmc| sub_id), 
                   data = bfi_data)


# checking normality of conditional residuals
qqnorm(residuals(l1_resid_mod, main="Q-Q plot for conditional residuals"))

# checking the normality of the random effects (here random intercept):
qqnorm(ranef(l1_resid_mod)$sub_id$bfi_self_pmc,
       main="Q-Q plot for the self random effect")

# Checking residuals for intercept
qqnorm(ranef(l1_resid_mod)$sub_id$`(Intercept)`,
       main="Q-Q plot for the random intercept")

```

Normal, normal, intercept is lightly tailed/normal
...
# mod2 <- aov(itt_comp_gmc ~ analog_condition, data = mod2_data) # Amw stats, does not matter that I used lm in mediation; probably easier for code
# summary(mod2)
