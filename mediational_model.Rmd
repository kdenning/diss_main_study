...
*In the full mediational model, should I just run 2b? It is truly what I am interested in, as I predicted differences depending on condition.*
...
mod3a_bfi <- lmer(bfi_targ ~ bfi_self_pmc*analog_condition*target_condition +
       (1 + bfi_self_pmc| sub_id), data = mod3_data_bfi)
summary(mod3a_bfi)
...
mod3b_bfi <- lmer(bfi_targ_pmc ~ bfi_self_pmc*analog_condition*target_condition*itt_comp_gmc +
       (0 + bfi_self_pmc| sub_id), data = mod3_data_bfi)
summary(mod3b_bfi)
...
Looks like I could do an LM since the random effect for eli_Self also has zero variance... Maybe a composite measure?

Regardless analog also has no effect.
...
df_coeffs <- data.frame(matrix(vector(mode = 'numeric',length = 10000), nrow = 10000, ncol = 12))

df_coeffs %<>% 
  rename(pt_on_threat_coeff = X1,
         pt_on_threat_cil =  X2,
         pt_on_threat_ciu = X3,
         threat_on_cp_coeff = X4,
         threat_on_cp_cil = X5,
         threat_on_cp_ciu = X6,
         pt_on_cp_coeff = X7,
         pt_on_cp_cil = X8,
...
for (i in 1:10) {
  ...
  coeff_pt_on_threat <- pt_on_threat$coefficients[2] # effect of analog on threat
  
  # Getting indirect CI
  lm_ci <- confint(pt_on_threat,level=0.95)
  pt_on_threat_cil <- lm_ci[2,1]
  pt_on_threat_ciu <- lm_ci[2,2]
  
  # Running model on direct effect
  ...
  coeff_threat_on_cp <- fixed_eff[5] # effect of threat on projection
  coeff_pt_on_cp <- fixed_eff[6] # effect of pt on projection
  coeff_int_on_cp <- fixed_eff[8] # interaction on projection
  
  # Getting CIs for lmer model
  lmer_ci <- confint(threat_on_cp,level=0.95)
  threat_on_cp_cil <- lmer_ci[9,1]
  threat_on_cp_ciu <- lmer_ci[9,2]
  pt_on_cp_cil <- lmer_ci[10,1]
  pt_on_cp_ciu <- lmer_ci[10,2]
  int_on_cp_cil <- lmer_ci[12,1]
  int_on_cp_ciu <- lmer_ci[12,2]
  
  # Putting coefficients into overall df
  df_coeffs$pt_on_threat_coeff[i] <- coeff_pt_on_threat
  df_coeffs$pt_on_threat_cil[i] <- pt_on_threat_cil
  df_coeffs$pt_on_threat_ciu[i] <- pt_on_threat_ciu
  df_coeffs$threat_on_cp_coeff[i] <- coeff_threat_on_cp
  df_coeffs$threat_on_cp_cil[i] <- threat_on_cp_cil
  df_coeffs$threat_on_cp_ciu[i] <- threat_on_cp_ciu
  df_coeffs$pt_on_cp_coeff[i] <- coeff_pt_on_cp
  df_coeffs$pt_on_cp_cil[i] <- pt_on_cp_cil
  df_coeffs$pt_on_cp_ciu[i] <- pt_on_cp_ciu
  df_coeffs$int_on_cp_coeff[i] <- coeff_int_on_cp
  df_coeffs$int_on_cp_cil[i] <- int_on_cp_cil
  df_coeffs$int_on_cp_ciu[i] <- int_on_cp_ciu
}
