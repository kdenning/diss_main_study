...
```{r}
threat_levels = list(itt_comp_c = c(-1.07, 0, 1.07))
mod1_simpslopes_bfi <- emtrends(model1_bfi_randslopes, ~ itt_comp_c,
                              var ="bfi_self_c",
                              at = threat_levels)
...
```{r}
mod1_bfi_maineffect <- effect("bfi_self_c:itt_comp_c",
                         xlevels = list(itt_comp_c = c(-1.07, 0, 1.07)),
                         mod = model1_bfi_randslopes)

mod1_bfi_maineffect <- as.data.frame(mod1_bfi_maineffect)
mod1_bfi_maineffect$itt_comp_c <- as.factor(mod1_bfi_maineffect$itt_comp_c)

ggplot(mod1_bfi_maineffect, aes(bfi_self_c, fit, group = itt_comp_c)) +
  geom_smooth(method = "lm", 
                size = .7, 
                se = FALSE,
                colour = "black", 
                aes(linetype = itt_comp_c)) +
...
```{r}
mod1_simpslopes_eli <- emtrends(model1_eli_randslopes, ~ itt_comp_c,
                              var ="eli_self_c",
                              at = threat_levels)
...
```{r}
mod1_eli_maineffect <- effect("eli_self_c:itt_comp_c",
                         xlevels = list(itt_comp_c = c(-1.07, 0, 1.07)),
                         mod = model1_eli_randslopes)

mod1_eli_maineffect <- as.data.frame(mod1_eli_maineffect)
mod1_eli_maineffect$itt_comp_c <- as.factor(mod1_eli_maineffect$itt_comp_c)

ggplot(mod1_eli_maineffect, aes(eli_self_c, fit, group = itt_comp_c)) +
  geom_smooth(method = "lm", 
                size = .7, 
                se = FALSE,
                colour = "black", 
...
```{r}
model1b_bfi_randslopes <- lmer(bfi_targ ~ bfi_self_c*itt_comp_c*target_condition + 
                     (0 + bfi_self_c | sub_id), 
                   data = mod_1_data_bfi) # Model only works with the whole data-set (clean_data), not when filtered down to only include observations (thus none repeated) for this analysis with mod_1_data_bfi
...
mod1b_simpslopes_bfi_int <- emtrends(model1b_bfi_randslopes, ~ target_condition*itt_comp_c,
                              var ="bfi_self_c",
...
```{r}
mod1b_bfi_int <- effect("bfi_self_c:itt_comp_c:target_condition",
                         xlevels = list(itt_comp_c = c(-1.07, 0, 1.07),
...
mod1b_bfi_int$itt_comp_c <- as.factor(mod1b_bfi_int$itt_comp_c)
mod1b_bfi_int$target_condition <- as.factor(mod1b_bfi_int$target_condition)

threat_labels <- c("-1.07" = "Low Threat",
                   "0" = "Average Threat",
                   "1.07" = "High Threat")

ggplot(mod1b_bfi_int, aes(bfi_self_c, fit, group = target_condition)) +
  geom_smooth(method = "lm", 
                size = .7, 
                se = FALSE,
                colour = "black", 
    ...
    theme_minimal(base_size = 13) +
    theme(legend.key.size = unit(1, "cm")) +
  facet_wrap(~itt_comp_c,
             labeller = labeller(condition_order = threat_labels))
  scale_linetype_manual("Target-level \nthreat",
                        breaks = c(-1.07, 0, 1.07), 
                       labels = c("Low",
                                  "Average",
