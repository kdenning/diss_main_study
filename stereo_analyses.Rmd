...
ggplot(bfi_no_stereo_df, aes(bfi_self_pmc, fit, group = target_condition)) +
...
ggplot(bfi_stereo_df, aes(bfi_self_pmc, fit, group = target_condition)) +
  geom_smooth(method = "lm", 
                size = .7, 
                se = FALSE,
                colour = "black", 
...
NEED TO UPDATE FOR ELI

### Simple Slopes

```{r}
simpslopes_eli_nostereo1 <- emtrends(eli_nostereo, ~ itt_comp_gmc,
                              var ="eli_self_pmc",
                              at = list(threat_levels,
                                        target_condition = c("CONTROL", 
                                                             "LOSS", 
                                                             "WARM")))
...
eli_nostereo <- lmer(eli_targ_pmc ~ eli_self_pmc*itt_comp_gmc*target_condition +
                     (0 + eli_self_pmc | sub_id), data = data_eli)
summary(eli_nostereo)
tab_model(eli_nostereo)
```
