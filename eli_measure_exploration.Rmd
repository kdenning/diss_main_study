...
simpslopes_eli_nostereo_compcond <- emtrends(compcond_eli_randslopes, ~ itt_comp_gmc*target_condition,
                              var ="eli_self_pmc",
...
```{r}
eli_nostereo_int <- effect("eli_self_pmc:itt_comp_gmc:target_condition",
                         xlevels = list(itt_comp_gmc = c(-1.07, 0.0, 1.07),
                                        target_condition = c("CONTROL",
                                                             "WARM",
                                                             "LOSS")),
                         mod = compcond_eli_randslopes)

eli_nostereo_int <- as.data.frame(eli_nostereo_int)
eli_nostereo_int$itt_comp_gmc <- as.factor(eli_nostereo_int$itt_comp_gmc)
eli_nostereo_int$target_condition <- as.factor(eli_nostereo_int$target_condition)
...
ggplot(eli_nostereo_int, aes(eli_self_pmc, fit, group = itt_comp_gmc)) +
  geom_smooth(method = "lm", 
                size = .7, 
                se = FALSE,
                colour = "black", 
  ...
  scale_linetype_manual("Threat composite",
    ...
    labs(title = "Projection by target-level threat and target condition",
         subtitle = "Accounting for stereotyping",
       x = "ELI responses for self",
       y = "ELI responses for target")
```
...
```{r}
threat_descrip <- describe(eli_data$itt_comp_gmc)
onesdlow <- threat_descrip$mean - threat_descrip$sd
onesdlow
onesdhigh <- threat_descrip$mean + threat_descrip$sd
onesdhigh
eli_descrip <- eli_data %>% 
  select(sub_id, eli_number, eli_targ_pmc, eli_self_pmc, target_condition, itt_comp_gmc) %>% 
  mutate(itt_comp_cat = as.factor(ifelse(itt_comp_gmc < onesdlow, "Low",
                               ifelse(itt_comp_gmc >= onesdlow & itt_comp_gmc <= onesdhigh, "Ave",
                                      ifelse(itt_comp_gmc > onesdhigh, "High", NA)))))
...
ggplot(eli_descrip, aes(eli_self_pmc, eli_targ_pmc, group = itt_comp_cat)) +
  geom_smooth(method = "lm", 
                size = .7, 
                colour = "black", 
              se = FALSE,
                aes(linetype = itt_comp_cat)) +
    theme_minimal(base_size = 13) +
    theme(legend.key.size = unit(1, "cm")) +
  facet_wrap(~target_condition,
             labeller = labeller(target_condition = target_labels))
...
### Responses in warm condition by threat comp

#### Low threat for self

```{r}
filter_warmlow <- eli_data %>% 
  filter(target_condition == "WARM" &
           itt_comp_gmc < -1.07)

hist(filter_warmlow$eli_self_pmc)
```

#### Low threat for target

```{r}
hist(filter_warmlow$eli_targ_pmc)
```

These are different... corresponds to why I am seeing counter-projection, but just doesn't make sense.

#### Ave threat for self

```{r}
filter_warmave <- eli_data %>% 
  filter(target_condition == "WARM" &
           itt_comp_gmc > -1.07 & itt_comp_gmc < 1.07)

hist(filter_warmave$eli_self_pmc)
```

#### Ave threat for target 

```{r}
hist(filter_warmave$eli_targ_pmc)
```

These are pretty similiar, which corresponds to why we are seeing projection

#### High threat for self

```{r}
filter_warmhigh <- eli_data %>% 
  filter(target_condition == "WARM" &
           itt_comp_gmc > 1.07)

hist(filter_warmhigh$eli_self_pmc)
```

#### High threat for target

```{r}
hist(filter_warmhigh$eli_targ_pmc)
```

Different enough that I'm not suprised we are seeing projection, but is less different than with the low threat... which just seems so weird
...
```{r}
simpslopes_eli_stereo_compcond <- emtrends(compcond_eli_stereo, ~ itt_comp_gmc*target_condition,
                              var ="eli_self_pmc",
                              at = c(targ_levels, threat_levels))
...
eli_stereo_int <- effect("eli_self_pmc:itt_comp_gmc:target_condition",
                         xlevels = list(itt_comp_gmc = c(-1.07, 0.0, 1.07),
                                        target_condition = c("CONTROL",
                                                             "WARM",
                                                             "LOSS")),
...
eli_stereo_int <- as.data.frame(eli_stereo_int)
eli_stereo_int$itt_comp_gmc <- as.factor(eli_stereo_int$itt_comp_gmc)
eli_stereo_int$target_condition <- as.factor(eli_stereo_int$target_condition)
...
ggplot(eli_stereo_int, aes(eli_self_pmc, fit, group = itt_comp_gmc)) +
  geom_smooth(method = "lm", 
                size = .7, 
                se = FALSE,
                colour = "black", 
