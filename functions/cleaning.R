...
get_wrangled_bfi <- function(data){
  clean <- data %>% 
  ...
  select(-c(drop1, self, ster, targ)) %>% 
    mutate_at(c("bfi_number", "bfi_self", "bfi_stereo",
                "bfi_targ"),
              list(~as.numeric(.))) %>% 
...
get_wrangled_eli <- function(data){
  clean <- data %>% 
    ...
    mutate_at(c("eli_number", "eli_self", "eli_stereo", "eli_targ"),
              list(~as.numeric(.))) %>% 
    group_by(sub_id) %>% 
    mutate(eli_self_mp = mean(eli_self, na.rm = T),
           eli_stereo_mp = mean(eli_stereo, na.rm = T),
           eli_targ_mp = mean(eli_targ, na.rm = T)) %>% 
    ungroup() %>% 
    mutate(eli_self_gmc = scale(eli_self, center = T, scale = F),
...
# Variable cleaning ------------------------------------------------------------
get_vars_cleaned <- function(data){
  clean <- data %>% 
